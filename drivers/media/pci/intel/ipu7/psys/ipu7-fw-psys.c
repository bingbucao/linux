...
#ifdef IPU_PSYS_COMPRESSION
static void
ipu7_fw_psys_compression_link(const struct ipu_psys_link_cmprs_conf *conf,
			      void **buf_ptr_ptr, u32 *buf_size)
{
	struct ipu7_msg_link_cmprs_option *cmprs_option =
		(struct ipu7_msg_link_cmprs_option *)(*buf_ptr_ptr);
	const struct ipu_psys_plane_cmprs *src_plane_desc;
	struct ipu7_msg_link_cmprs_plane_desc *dst_plane_desc;
	struct ia_gofo_tlv_header *header = &cmprs_option->header;
	u32 cmprs_option_size = sizeof(*cmprs_option);

	cmprs_option->cmprs_buf_size = conf->cmprs_buf_size;
	cmprs_option->align_interval = conf->align_interval;

	for (int i = 0; i < 2; i++)
		cmprs_option->reserved[i] = 0;

	for (int i = 0; i < IPU_MSG_LINK_CMPRS_MAX_PLANES; i++) {
		dst_plane_desc = &cmprs_option->plane_descs[i];
		src_plane_desc = &conf->plane_cmprs[i];

		dst_plane_desc->plane_enable = src_plane_desc->plane_enable;
		dst_plane_desc->cmprs_enable = src_plane_desc->cmprs_enable;
		dst_plane_desc->encoder_plane_id =
			src_plane_desc->encoder_plane_id;
		dst_plane_desc->decoder_plane_id =
			src_plane_desc->decoder_plane_id;
		dst_plane_desc->cmprs_is_lossy = src_plane_desc->is_lossy;
		dst_plane_desc->cmprs_is_footprint =
			src_plane_desc->is_footprint;
		dst_plane_desc->bit_depth = src_plane_desc->bpp0;
		dst_plane_desc->space_saving_numerator =
			src_plane_desc->space_saving_numerator;
		dst_plane_desc->pixels_offset = src_plane_desc->pixels_offset;
		dst_plane_desc->ts_offset = src_plane_desc->ts_offset;
		dst_plane_desc->tile_row_to_tile_row_stride =
			src_plane_desc->row_of_tiles_stride;
		dst_plane_desc->rows_of_tiles = src_plane_desc->rows_of_tiles;
		dst_plane_desc->lossy_cfg[0] =
			src_plane_desc->lossy_config0.value;
		dst_plane_desc->lossy_cfg[1] =
			src_plane_desc->lossy_config1.value;
		dst_plane_desc->lossy_cfg[2] =
			src_plane_desc->lossy_config2.value;
		dst_plane_desc->lossy_cfg[3] =
			src_plane_desc->lossy_config3.value;
		dst_plane_desc->lossy_cfg[4] =
			src_plane_desc->lossy_config4.value;
	}

	header->tlv_type = TLV_TYPE(IPU_MSG_LINK_OPTION_TYPES_CMPRS);
	header->tlv_len32 = TLV_SIZE(cmprs_option_size);

	*buf_ptr_ptr += cmprs_option_size;
	*buf_size += cmprs_option_size;
}
				    ...
				    void **buf_ptr_ptr)
{
	...
	*buf_ptr_ptr += sizeof(*msg_link);
	...
	if (link->link_cmprs_conf.enable) {
		ipu7_fw_psys_compression_link(&link->link_cmprs_conf,
					      buf_ptr_ptr, &buf_size);
		msg_link->link_options.num_elems++;
	}

#endif
	msg_link->tlv_header.tlv_type = TLV_TYPE(IPU_MSG_LINK_TYPE_GENERIC);
#ifdef IPU_PSYS_COMPRESSION
	msg_link->tlv_header.tlv_len32 = TLV_SIZE(buf_size);
#else
