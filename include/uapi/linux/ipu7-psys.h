...
/* For lossy compression config 32bits for each */
struct lossyConf0 {
	union {
		struct {
			uint32_t lossy_ratio_plus : 8;
			uint32_t lossy_ratio_mins : 8;
			uint32_t lossy_inst_upper : 8;
			uint32_t lossy_inst_lower : 8;
		} bits;
		uint32_t value;
	};
} __attribute__ ((packed));
...
struct lossyConf2 {
	union {
		struct {
			uint32_t init_qp : 5; //[4:0]
			uint32_t rsd1 : 3;
			uint32_t max_qp : 5; //[12:8]
			uint32_t rsd2 : 3;
			uint32_t min_qp : 5; //[20:16]
			uint32_t rsd3 : 3;
			uint32_t window_size : 2; //[25:24]
			uint32_t rsd : 6;
		} bits;
		uint32_t value;
	};
...
struct lossyConf3 {
	union {
		struct {
			uint32_t rsd1 : 8;
			uint32_t max_qp_inc : 4; //[11:8]
			uint32_t rsd2 : 4;
			uint32_t max_qp_dec : 4; //[19:16]
			uint32_t rsd3 : 4;
			uint32_t qp_inc_reset : 3; //[26:24]
			uint32_t rsd4 : 5;
		} bits;
		uint32_t value;
	};
...
struct lossyConf4 {
	union {
		struct {
			uint32_t log_foot_print_guard_band : 4; //[3:0]
			uint32_t rsd1 : 28;
		} bits;
		uint32_t value;
	};
...
/**
 * struck PsysPlaneCompressionConf - Plane compression description
 * The information are for both lossless and lossy mode.
 */
struct ipu_psys_plane_cmprs {
	uint8_t plane_enable;
	uint8_t cmprs_enable;
	uint16_t rows_of_tiles;
	uint32_t pixels_offset;
	uint32_t ts_offset;
	uint32_t row_of_tiles_stride;
	uint8_t bpp0;
	uint8_t encoder_plane_id;
	uint8_t decoder_plane_id;
	uint8_t is_lossy;
	uint8_t is_footprint;
	uint8_t space_saving_numerator;
	struct lossyConf0 lossy_config0;
	struct lossyConf1 lossy_config1;
	struct lossyConf2 lossy_config2;
	struct lossyConf3 lossy_config3;
	struct lossyConf4 lossy_config4;
};

/**
 * struck psys_link_compression_conf - Compression setting are optional
 * for links.Each frame has two plane.
 */
struct ipu_psys_link_cmprs_conf {
	uint8_t enable;
	uint32_t cmprs_buf_size;
	uint16_t align_interval;
	uint8_t reserved[2];  // Reserved for alignment.  Set to zero.
	struct ipu_psys_plane_cmprs plane_cmprs[IPU_MSG_LINK_CMPRS_MAX_PLANES];
};
#endif
 ...
 * @delayed_link:	A delay link between producer N and consumer N+1 frame
 #ifdef IPU_PSYS_COMPRESSION
 * @reserved:		Align to 32 bits.  Set to zero
 * @link_cmprs_conf:	Extension fields for links
 #endif
 */
struct graph_link {
...
#ifdef IPU_PSYS_COMPRESSION
	uint8_t reserved[2];
	struct ipu_psys_link_cmprs_conf link_cmprs_conf;
#endif
} __attribute__ ((packed));
